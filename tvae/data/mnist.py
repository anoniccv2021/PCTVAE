...
from tvae.data.transforms import AddPerspectiveTransformationDims, AddRandomTransformationDims, AddFixedTransformationDims, AddDualTransformationDims, To_Color
from tvae.data.preprocessing import zca_whitening_matrix, NormalizePixels, StatsRecorder, ZCATransformation
    ...
    def compute_zca_matrix(self):
        if self.whiten:
            if not os.path.exists(self.processed_zca_path):
                ...
                flat_processed_train_images = np.array([np.array(data_train[i][0]).reshape(-1) for i in tqdm(range(len(data_train)))])
