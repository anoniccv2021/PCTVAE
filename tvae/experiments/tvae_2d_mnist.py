...
from tvae.models.fc import FC_Encoder_w_WN, FC_Decoder_w_WN
...
def create_model(n_caps, cap_dim, group_kernel, mu_init):
    ...
    z_encoder = Gaussian_Encoder(MLP_Encoder_w_WN(s_dim=s_dim, n_cin=1, n_hw=28),
                                 loc=0.0, scale=1.0)

    u_encoder = Gaussian_Encoder(MLP_Encoder_w_WN(s_dim=s_dim, n_cin=1, n_hw=28),                                
                                 loc=0.0, scale=1.0)

    decoder = Bernoulli_Decoder(MLP_Decoder_w_WN(s_dim=s_dim, n_cout=1, n_hw=28))
...
def main():
    ...
    name = 'TVAE-2D_MNIST_L=0_K=5x5_PWN_3LayerMLP_T=1/sqrt(WU**2)'
