...
def create_model(n_caps, cap_dim, mu_init, n_transforms, group_kernel, n_off_diag):
    ...
    z_encoder = Gaussian_Encoder(MLP_Encoder_w_WN(s_dim=s_dim, n_cin=1, n_hw=64),
                                 loc=0.0, scale=1.0)

    u_encoder = Gaussian_Encoder(MLP_Encoder_w_WN(s_dim=s_dim, n_cin=1, n_hw=64),                                
                                 loc=0.0, scale=1.0)

    decoder = Bernoulli_Decoder(MLP_Decoder_w_WN(s_dim=s_dim, n_cout=1, n_hw=64))
...
def main():
    ...
    name = 'TVAE_dSprites_L=1/2_K=1_PWN_reweight'
    ...
    for e in range(config['max_epochs']):
        ...
        total_loss, total_neg_logpx_z, total_kl, total_eq_loss, total_dis_corr, num_batches = train_epoch_dsprites(model, optimizer, 
                                                                     ...
                                                                     plot_transform_selectivity=True,
                                                                     plot_allcapoffsets=True, 
