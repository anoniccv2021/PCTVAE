...
from tvae.containers.tvae import PCTVAE
...
from tvae.containers.grouper import Causal_Capsules_from_Gaussian_1d
from tvae.utils.logging import configure_logging, get_dirs
from tvae.utils.train_loops import train_epoch, eval_epoch, final_results

def create_model(n_caps, cap_dim, mu_init, n_transforms, group_kernel, n_off_diag, config):
    s_dim = n_caps * cap_dim
    z_encoder = Gaussian_Encoder(MLP_Encoder(s_dim=s_dim, n_cin=3, n_hw=28),
                                  loc=0.0, scale=1.0)
   
    u_cap_dim = cap_dim # (cap_dim + group_kernel[1] // 2-1)
    u_dim = n_caps * u_cap_dim
    u_encoder = Gaussian_Encoder(MLP_Encoder(s_dim=u_dim, n_cin=3, n_hw=28),         
                                      loc=0.0, scale=1.0)

    decoder = Bernoulli_Decoder(MLP_Decoder(s_dim=s_dim, n_cout=3, n_hw=28))

    grouper = Causal_Capsules_from_Gaussian_1d(
                      nn.Conv3d(in_channels=1, out_channels=1,
                                          kernel_size=group_kernel, 
                                          padding=(0,0,0),
                                          stride=(1,1,1), padding_mode='zeros', bias=False),
                      lambda x: F.pad(x, (0, 0,
                                            group_kernel[1]//2-1, group_kernel[1]//2,
                                            group_kernel[0]//2-1, group_kernel[0]//2), 
                                            mode='circular'),
                    n_caps=n_caps, u_cap_dim=u_cap_dim, z_cap_dim=cap_dim, n_transforms=n_transforms,
                    mu_init=mu_init, n_off_diag=n_off_diag)
    
    return PCTVAE(z_encoder, u_encoder, decoder, grouper, 
                  bsz=config['batch_size'],
                  n_t=config['n_transforms'], 
...
def main():
    ...
    name = 'PCTVAE_MNIST_Causal_L=18/36(9)_K=3_FullCircSeq'
    ...
    is_by_dt = final_results(model, test_loader, log, n_is_samples=10, dt_set=range(0, 9, 1), wandb_on=True)
    print(is_by_dt)
    for dt in is_by_dt:
